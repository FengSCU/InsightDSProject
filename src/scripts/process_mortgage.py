...
def save_mortgage_data_for_city(spark, city, year, df):
    ...
    dest = "s3a://insight-project/productionv2_mortgage_detail/%s/%s/" % (city, year)
...
def calculate_mortgage_tract(spark, config, city, year, df):
    ...
    mortgage_data_table_name = config.get_mortgage_data_table_name_for_city(city)
    postgres.save_to_DB(spark, df_mortgage_w_level, mortgage_data_table_name, 'append')
    ...
    temp_view = 'df_mortgage_aggr_%s' % city
    df_mortgage_aggr.createTempView(temp_view)
    ...
    mortgage_rank_table_name = config.get_mortgage_rank_table_name_for_city(city)
    postgres.save_to_DB(spark, df_mortgage_rank, mortgage_rank_table_name, "append")
...
def process_mortgage_data(spark, config, city, year_to_process, df):
    ...
    df_mortgage_rank = calculate_mortgage_tract(spark, config, city, year_to_process, df_transform)
    return df_mortgage_rank
